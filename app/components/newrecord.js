...
export default function RegisterHealthRecordComponent({ search }) {
  ...
  const handleSubmit = async (e) => {
    ...
    data.set('file', file);
    try {
      let result = await axios.post("/api/records/newrecord", {
        ...formData,
        patientId,
        doctorId,
        method: "POST",
      ...
      result = await result.data; // Removed .json() as axios already parses the JSON response
      console.log(result);
      if (result.success) {
        alert("file upload");
      }
      ...
      console.log("HealthRecord Registration failed", error);
      toast.error("HealthRecord Registration failed");
    }
  };
